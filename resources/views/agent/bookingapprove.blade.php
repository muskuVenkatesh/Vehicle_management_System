...
@section('content')
            ...
            <tbody>
                @foreach ($bookings as $index => $booking)
                    <tr>
                        ...
                        <td>
                            <select class="status-dropdown" onchange="updateBookingStatus({{ $booking->id }}, this)">
    ...
    <script>
        ...
        function updateBookingStatus(bookingId, element) {
    let status = element.value;

    $.ajax({
        url: "{{ url('/bookings/update-status') }}/" + bookingId,
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
